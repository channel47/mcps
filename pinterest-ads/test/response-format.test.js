...
describe('formatSuccess', () => {
  test('wraps payload in MCP text content with row count metadata', () => {
...
describe('formatError', () => {
  test('maps explicit invalid-params errors to InvalidParams', () => {
    assert.throws(
      () => formatError(invalidParamsError('Missing required parameter: ad_account_id')),
      (error) => {
  ...
  test('treats untyped API errors as InternalError', () => {
    assert.throws(
      () => formatError(new Error('Pinterest Ads API request failed (500): upstream error')),
      (error) => {
